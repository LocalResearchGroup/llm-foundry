...
This assumes you have modal installed and have logged in via command line. Make sure that [modal_script.py](https://github.com/LocalResearchGroup/llm-foundry/blob/main/scripts/modal/modal_script.py) is in the same folder as the Dockerfile below:
...
In your modal_script.py make sure `pull_to_folder.remote()` is uncommented. That function downloads all of our datasets to Modal. If you want to train, make sure the three lines related to training are uncommented as well. Once you have [`modal` installed](https://modal.com/docs/guide#:~:text=Create%20an%20account,modal%20setup)) run:

```
MODAL_GPU=A100-40GB TRAIN_YAML=smollm2-135m_lora_pretraining.yaml IS_PEFT=true OUTPUT_PRECISION=bf16 modal run main.py
```

Change `MODAL_GPU` and `TRAIN_YAML` to whatever you'd like. `IS_PEFT=true` as the example YAML file is configured for LoRA. If all goes well you should see training logs wherever you're logging (W&B, AIM) and see printed logs in the terminal.
