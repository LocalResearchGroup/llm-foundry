...
New scripts for preprocessing pretraing (finemath and pythonedu) and finetuning (tulu, numina and glaive) datasets, tokenizing, and uploading/downloading datasets to Hugging Face.

The different command line utils provide arguments like `datasets` to target only one or a list of valid datasets, the `user_org` to override default organization in HF and you can generate tokenization for `decontaminated` datasets.

### `text_dataset_preproc.py` (only for finetuning datasets before calling text_dataset_tokenize)

Preprocesses is needed only for instruct datasets (Tulu, Numina, Glaive) by applying ChatML templates, filtering messages, and pushing to HF.

| Argument | Default | Description |
|----------|---------|-------------|
| `--datasets` | all instruct | Datasets to process (`tulu`, `numina`, `glaive`) |
| `--user_org` | `LocalResearchGroup` | HF user/org namespace |
| `--decontaminated` / `--no-decontaminated` | `--no-decontaminated` | Use decontaminated dataset instead of original |

### `text_dataset_tokenize.py` (only pretrain datasets)

Tokenizes pretrain and instruct datasets using `SmolLM2-135M` (or `-instruct`), outputs MDS format, and uploads to HF.

| Argument | Default | Description |
|----------|---------|-------------|
| `--datasets` | all datasets | Datasets to process (`tulu`, `numina`, `glaive`, `finemath`, `pythonedu`) |
| `--user_org` | `LocalResearchGroup` | HF user/org namespace |
| `--tokenize` / `--no-tokenize` | `--tokenize` | Generate local tokenization for splits |
| `--upload-tokens` / `--no-upload-tokens` | `--upload-tokens` | Upload local tokenization to HF |
| `--decontaminated` / `--no-decontaminated` | `--no-decontaminated` | Use decontaminated dataset instead of original |

### `download_tokens.py` (can be used to download tokenized de/contaminated versions generated by `text_dataset_tokenize`)

Downloads tokenized datasets from HF to local.

| Argument | Default | Description |
|----------|---------|-------------|
| `--datasets` | all datasets | Datasets to download (`tulu`, `numina`, `glaive`, `finemath`, `pythonedu`) |
| `--user_org` | `LocalResearchGroup` | HF user/org containing tokenizations |
| `--out` | `.` | Local download folder |
| `--decontaminated` / `--no-decontaminated` | `--no-decontaminated` | Download decontaminated variant instead of original |

### scripts/data_prep/data_lib/utils.pytest

Shared utilities: dataset configs, HF repo mappings, constants generation.

• `register_new_datasets(target)` — Allows us to registers new datasets constants (total rows, chars per sample/token) into CONSTS which is not exposed directly.
• `add_dataset_config(name, splits)` — adds a dataset to CONSTS.
• `get_datasets()` — returns dataset config dict (tulu, numina, glaive, finemath, pythonedu) with original/decontaminated HF repo paths, kind, and template info.
• `rel_path(name, decontaminated)` — builds dataset folder name with -with-template and/or -decontaminated suffixes.
• `generate_constants(total_rows, chars_per_sample, chars_per_token)` — creates DatasetConstants with train/test split info.
• `_banner(msg)`, `str_rows_features(ds)` — logging helpers.
